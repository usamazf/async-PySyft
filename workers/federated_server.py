    ...
    def fit(self, dataset_key: str, epoch: int, device: str = "cpu", return_ids: List[int] = None):
        ...
        Args:
            ...
            **kwargs:
                return_ids: List[str]
        """
        return_ids = return_ids if return_ids is not None else [sy.ID_PROVIDER.pop()]

        self._send_msg_and_deserialize("fit", return_ids=return_ids, dataset_key=dataset_key, epoch=epoch, device=device)
